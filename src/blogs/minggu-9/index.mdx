...
members:
  [
    ...
    { name: "Nabil Aziz Bima Anggita", as: "Scrum Master / Pembuat Laporan / Publisher" },
...
<code style="white-space: pre-wrap;">
  wget https://rules.emergingthreats.net/open/suricata/emerging.rules.tar.gz tar
  zxf emerging.rules.tar.gz sudo cp -r rules /etc/suricata/
</code>
...
<p>
  ...
  pengguna dapat membuat <i>dashboard Kibana</i> yang memungkinkan pengguna untuk
  mencari, membuat grafik, menganalisis, dan memperoleh wawasan dari <i>log</i>
  yang dihasilkan.
</p>
...
<p>
  ...
  , Pengguna dapat melakukan deteksi intrusi jaringan untuk berbagai ancaman.{" "}
  <i>Dashboard</i> ini memungkinkan pengguna untuk dengan cepat melihat
  kecenderungan dan anomali di dalam jaringan, serta menggali data untuk
  menemukan akar penyebab peringatan seperti agen pengguna yang berbahaya atau{" "}
  <i>port</i> yang rentan. Dengan data yang diekstraksi ini, pengguna dapat
  membuat keputusan berdasarkan informasi tentang cara bereaksi dan melindungi
  jaringan pengguna dari upaya penyusupan yang berbahaya, dan membuat aturan
  untuk mencegah penyusupan di masa mendatang ke jaringan pengguna.
</p>
